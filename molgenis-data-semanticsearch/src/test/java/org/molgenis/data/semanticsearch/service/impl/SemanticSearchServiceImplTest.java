...
public class SemanticSearchServiceImplTest extends AbstractMolgenisSpringTest
{
	...
	{
		EntityType sourceEntityMetaData = entityTypeFactory.create().setLabel("sourceEntityMetaData");
		Attribute sourceAttributeHeight = attrMetaDataFactory.create().setName("height_0").setLabel("height in cm");
		Attribute sourceAttributeWeight = attrMetaDataFactory.create().setName("weight_0").setLabel("weight in kg");
		...
		when(sourceEntity1.getString(AttributeMetadata.ID)).thenReturn("1");
		when(sourceEntity1.getString(AttributeMetadata.NAME)).thenReturn("height_0");
		when(sourceEntity1.getString(AttributeMetadata.LABEL)).thenReturn("height");
		when(sourceEntity1.getString(AttributeMetadata.TYPE)).thenReturn(AttributeType.DECIMAL.toString());
		when(sourceEntity1.getString(AttributeMetadata.DESCRIPTION)).thenReturn("this is a height measurement in m!");

		Entity sourceEntity2 = mock(Entity.class);
		when(sourceEntity2.getString(AttributeMetadata.ID)).thenReturn("2");
		when(sourceEntity2.getString(AttributeMetadata.NAME)).thenReturn("weight_0");
		when(sourceEntity2.getString(AttributeMetadata.LABEL)).thenReturn("weight");
		when(sourceEntity2.getString(AttributeMetadata.TYPE)).thenReturn(AttributeType.DECIMAL.toString());
		when(sourceEntity2.getString(AttributeMetadata.DESCRIPTION)).thenReturn("weight measured in kg");

		Entity sourceEntityMetaDataEntity = mock(Entity.class);

		when(sourceEntityMetaDataEntity.getEntities(EntityTypeMetadata.ATTRIBUTES))
				.thenReturn(Arrays.asList(sourceEntity1, sourceEntity2));

		when(dataService.findOne(ENTITY_TYPE_META_DATA,
				new QueryImpl<Entity>().eq(EntityTypeMetadata.ID, sourceEntityMetaData.getId())))
		...
		List<QueryRule> disMaxQueryRules = Lists
				.newArrayList(new QueryRule(AttributeMetadata.ID, QueryRule.Operator.IN, Arrays.asList("1", "2")),
		...
		// Case 1
		when(dataService.findAll(ATTRIBUTE_META_DATA, new QueryImpl<Attribute>(disMaxQueryRules), Attribute.class))
				.thenReturn(Stream.of(sourceAttributeHeight));

		Map<Attribute, ExplainedMatchCandidate<Attribute>> termsActual1 = semanticSearchService
				.findAttributes(sourceEntityMetaData, lexicalQueries, tagGroups);

		Map<Attribute, ExplainedMatchCandidate<Attribute>> termsExpected1 = ImmutableMap.of(sourceAttributeHeight,
		...
		Map<Attribute, ExplainedMatchCandidate<Attribute>> termsActual2 = semanticSearchService
				.findAttributes(sourceEntityMetaData, lexicalQueries, tagGroups);

		Map<Attribute, ExplainedMatchCandidate<Attribute>> termsExpected2 = ImmutableMap.of();
	...
	public void testFindTags()
	{
		...
		when(dataService.getEntityType("sourceEntityMetaData")).thenReturn(sourceEntityMetaData);
		...
		Map<Attribute, Hit<OntologyTagObject>> expected = new LinkedHashMap<Attribute, Hit<OntologyTagObject>>();
		...
		Map<Attribute, Hit<OntologyTagObject>> actual = semanticSearchService
