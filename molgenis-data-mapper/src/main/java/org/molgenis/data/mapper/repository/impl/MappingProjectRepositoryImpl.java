...
public class MappingProjectRepositoryImpl implements MappingProjectRepository
{
	...
	public void add(MappingProject mappingProject)
	{
		...
		dataService.add(mappingProjectMetaData.getId(), toEntity(mappingProject));
	}
	...
	public void update(MappingProject mappingProject)
	{
		...
		dataService.update(mappingProjectMetaData.getId(), mappingProjectEntity);
	}

	@Override
	public MappingProject getMappingProject(String identifier)
	{
		Entity mappingProjectEntity = dataService.findOneById(mappingProjectMetaData.getId(), identifier);
	...
	public List<MappingProject> getAllMappingProjects()
	{
		...
		dataService.findAll(mappingProjectMetaData.getId()).forEach(entity ->
		{
	...
	{
		List<MappingProject> results = new ArrayList<>();
		dataService.findAll(mappingProjectMetaData.getId(), q).forEach(entity ->
		{
			results.add(toMappingProject(entity));
		});
		return results;
	...
	public void delete(String mappingProjectId)
	{
		...
		if (mappingProject != null)
		{
			...
			dataService.deleteById(mappingProjectMetaData.getId(), mappingProjectId);
