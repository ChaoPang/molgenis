...
public class MappingServiceImpl implements MappingService
{
	...
	public MappingServiceImpl(DataService dataService, AlgorithmService algorithmService, IdGenerator idGenerator,
			...
			PermissionSystemService permissionSystemService, AttributeFactory attributeFactory,
			...
			Boolean addSourceAttribute)
	{
		EntityType targetMetadata = EntityType
				.newInstance(mappingTarget.getTarget(), DEEP_COPY_ATTRS, attributeFactory);
		...
		if (Boolean.TRUE.equals(addSourceAttribute))
		{
			targetMetadata.addAttribute(attributeFactory.create().setName(SOURCE));
		}
	...
	private long applyMappingsInternal(MappingTarget mappingTarget, Repository<Entity> targetRepo, Progress progress)
	{
		...
		if (EntityTypeUtils.hasSelfReferences(targetRepo.getEntityType()))
		{
	...
	private void compareTargetMetadatas(EntityType targetRepositoryEntityType, EntityType mappingTargetEntityType)
	{
		...
		for (Attribute mappingTargetAttribute : mappingTargetEntityType.getAtomicAttributes())
		{
			...
			if (EntityTypeUtils.isReferenceType(mappingTargetAttribute))
			{
	...
	{
		Object id;
		if (dataType == AttributeType.INT || dataType == AttributeType.LONG || dataType == AttributeType.DECIMAL)
		{
			id = count + 1;
		}
		else
