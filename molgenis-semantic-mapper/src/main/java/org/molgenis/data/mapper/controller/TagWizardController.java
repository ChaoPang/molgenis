...
public class TagWizardController extends MolgenisPluginController
{
	...
	{
		List<String> entityTypeIds = dataService.findAll(ENTITY_TYPE_META_DATA, EntityType.class).map(EntityType::getId)
				.collect(toList());
		...
		Map<String, Multimap<Relation, OntologyTagObject>> taggedAttributes = attributes.stream()
	...
	OntologyTag addTagAttribute(@Valid @RequestBody AddTagRequest request)
	{
		return ontologyTagService
				.addAttributeTag(request.getEntityTypeId(), request.getAttributeName(), request.getRelationIRI(),
	...
	{
		ontologyTagService
				.removeAttributeTag(request.getEntityTypeId(), request.getAttributeName(), request.getRelationIRI(),
	...
	{
		Map<Attribute, Hit<OntologyTagObject>> autoGeneratedTags = semanticSearchService
				.findTags(request.getEntityTypeId(), request.getOntologyIds());
