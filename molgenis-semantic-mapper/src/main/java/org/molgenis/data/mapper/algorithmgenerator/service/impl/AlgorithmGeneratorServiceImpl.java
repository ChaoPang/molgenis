...
public class AlgorithmGeneratorServiceImpl implements AlgorithmGeneratorService
{
	...
	public GeneratedAlgorithm generate(Attribute targetAttribute,
			Map<Attribute, ExplainedMatchCandidate<Attribute>> sourceAttributes, EntityType targetEntityType,
			EntityType sourceEntityType)
	{
		...
		if (sourceAttributes.size() > 0)
		{
			...
			{
				Entry<Attribute, ExplainedMatchCandidate<Attribute>> firstEntry = sourceAttributes.entrySet().stream()
						.findFirst().get();
